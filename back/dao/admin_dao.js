...
module.exports = class admin_dao extends require('../model/admin_mod') {
  ...
  static async getUsersByTypeAndChar(req, resp) {
    ...
    let data = await this.getUsersByTypeAndCharMod(
      type,
      inputText,
      CharType,
      currPage,
  ...
  static async announce(req, resp) {
    ...
    let results = await this.announceMod(title, content, classes)
    resp.send(results)
  }
  ...
  static async getLeave(req, resp) {
    ...
    let data = await this.getLeaveMod(
      classArr,
      req.query.currPage,
      req.query.pageNum
    )
    let students = await this.getStudents(data.map(item => item.u_id))
    let total = await this.getLeaveTotal(classArr)
    resp.send({ data, students, total: total[0].count })
  }
  ...
  static async getuserLeave(req, resp) {
    let verify = await jwtUtil.verifysync(req.query.token, globalKey)
    let u_id = verify.id
    let data = await this.getuserLeaveMod(
      u_id,
      req.query.currPage,
      req.query.pageNum
    )
  ...
  static async getClassesSear(req, resp) {
    ...
    let data = await this.getClassesSearMod(
      classes,
      req.query.pageNum,
      req.query.currPage
    )
  ...
  static async getNoticeSear(req, resp) {
    let title = req.query.title
    let data = await this.getNoticeSearMod(
      title,
      req.query.pageNum,
      req.query.currPage
    )
    let total = await this.getNoticeSearTotal(title)
    console.log(data)
    resp.send({ data, total: total[0].count })
  }

  static async addTeacher(req, resp) {
    ...
    let result = await this.addTeacherMod(
      name,
      sex,
      address,
      position,
  ...
  static async deleteClass(req, resp) {
    let c_id = req.query.c_id
    let result = await this.deleteClassesMod(c_id)
    resp.send(result)
  }

  static async updateClass(req, resp) {
    let c_id = req.query.c_id
    let classes = req.query.classes
    let result = await this.updateClassMod(c_id, classes)
    resp.send(result)
  }

  static async addStudent(req, resp) {
    let name = req.query.username
    let sex = req.query.sex
    let address = req.query.address
    let classes = req.query.classes
    ...
    let type = req.query.type
    let result = await this.addStudentMod(name, sex, address, classes, type)
    resp.send(result)
  }
}
