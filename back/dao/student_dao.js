...
module.exports = class student_dao extends require('../model/student_mod') {
  ...
  static async getNotice(req, resp) {
    ...
    let data = await this.getNoticeMod(classes, pageNum, currPage)
    let total = await this.getNoticeTotal(classes)
  ...
  static async received(req, resp) {
    let verify = await jwtUtil.verifysync(req.query.token, globalKey)
    let u_id = verify.id
    let data = await this.getNoticeReadMod(u_id)
    resp.send(data)
  ...
  static async sethealth(req, resp) {
    ...
    let u_id = verify.id
    let createtime = this.getNowAndLastDate()
    let data = await this.sethealthMod(
      u_id,
      temperature,
      hot,
      gohubei,
  ...
  static async getHealthMonthByid(req, resp) {
    let startDate = new Date(req.query.startDate)
    let endDate = startDate.setDate(42) // 结束时间设置为从startDate开始向后数42天
    let verify = await jwtUtil.verifysync(req.query.token, globalKey)
    let u_id = verify.id
    let data = await this.getHealthMonthByidMod(u_id, startDate, endDate)
    resp.send(data)
  }
