module.exports = class admin_mod extends require('./model') {
   ...
   * @returns {Promise<any>}
   */
  static getUsersByTypeAndCharMod(
    type,
    inputText,
    CharType,
    currPage,
   ...
   * @returns {Promise<any>}
   */
  static announceMod(title, content, classes) {
    return new Promise((resolve, reject) => {
      let sql =
        'insert into `notice` (title,content,class,createtime) values (?,?,?,?)'
      this.query(sql, this.formatParams(title, content, classes, Date.now()))
        .then(result => {
          resolve('发布成功')
        })
        .catch(err => {
  ...
  static getStudents(u_id) {
    u_id = [...new Set(u_id)]
    let ids = u_id.join(',')
    return new Promise(resolve => {
      let sql = `select id,username from user where id in(${ids})`
  ...
  static getuserLeaveMod(u_id, currPage, pageNum) {
    ...
    return new Promise((resolve, reject) => {
      let sql =
        'select * from `leave` where u_id= ? order by createtime desc limit ?,? '
      this.query(sql, this.formatParams(u_id, currPage, pageNum))
        .then(result => {
          resolve(result)
        })
        .catch(err => {
  ...
  static upLeaveStateMod(l_id, upState) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.query(sql, this.formatParams(upState, l_id))
        .then(result => {
          resolve('审批成功')
        })
        .catch(err => {
  ...
  static getreadByidArr(readIdArr) {
    ...
    for (let i = 0; i < readIdArr.length; i++) {
      if (i == 0)
        sql +=
          'select id,username,head,classes,createtime from `user` where  id=  ' +
          readIdArr[i].u_id
  ...
  static NoticeDetailsTotal(classes) {
    ...
    for (let i = 0; i < classesArr.length; i++) {
      if (i == 0)
        sql +=
          "select count(1) as count from `user` where classes= '" +
          classesArr[i] +
          "'"
      else sql += " or classes = '" + classesArr[i] + "'"
    }
    return new Promise((resolve, reject) => {
      this.query(sql)
        .then(result => {
  ...
  static deleteClassesMod(c_id) {
    return new Promise((resolve, reject) => {
      let sql = 'delete from `class` where c_id=?'
      this.query(sql, this.formatParams(c_id))
        .then(res => {
          resolve(res)
        })
        .catch(() => {
  ...
  static updateClassMod(c_id, classes) {
    return new Promise((resolve, reject) => {
      let sql = 'update `class` set classes=? where c_id=?'
      this.query(sql, this.formatParams(classes, c_id))
        .then(res => {
          resolve(res)
        })
        .catch(() => {
  ...
  static addClassesMod(classAll, classes) {
    return new Promise((resolve, reject) => {
      ...
      this.query(sql, this.formatParams(classes))
        .then(() => {
          resolve('添加成功')
        })
        .catch(() => {
  ...
  static getClassesSearMod(classes, pageNum, currPage) {
    ...
    return new Promise((resolve, reject) => {
      let sql =
        "select * from `class` where classes like '%" + classes + "%' limit ?,?"
      this.query(sql, this.formatParams(currPage, pageNum))
        .then(result => {
          resolve(result)
        })
  ...
  static getNoticeSearMod(title, pageNum, currPage) {
    pageNum = Number(pageNum)
    currPage = Number(currPage)
    currPage = Number(pageNum * currPage)
    return new Promise((resolve, reject) => {
      let sql =
        "select * from `notice` where title like '%" + title + "%' limit ?,?"
  ...
  static getClassesSearTotal(classes) {
    return new Promise((resolve, reject) => {
      let sql =
        "select count(1) as count from `class` where classes like '%" +
        classes +
        "%' "
      this.query(sql)
        .then(result => {
  ...
  static getNoticeSearTotal(title) {
    console.log(title)
    return new Promise(resolve => {
      let sql =
        "select count(1) as count from `notice` where title like '%" +
        title +
        "%' "
  ...
  static addTeacherMod(name, sex, address, position, type, classes) {
    return new Promise((resolve, reject) => {
      let sql = `insert into user(username,sex,address,position,type,classes,createtime) values(?,?,?,?,?,?,?)`
      this.query(
        sql,
        this.formatParams(
          name,
  ...
  static addStudentMod(name, sex, address, classes, type) {
    return new Promise((resolve, reject) => {
      let sql = `insert into user(username,sex,classes,address,type) values(?,?,?,?,?)`
      this.query(sql, this.formatParams(name, sex, classes, address, type))
        .then(result => {
          resolve(result)
        })
        .catch(() => {
