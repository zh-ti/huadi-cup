module.exports = class student_mod extends require('./model') {
  ...
  static getNoticeTotal(u_classes) {
    return new Promise((resolve, reject) => {
      let sql =
        "select count(1) as count from notice  where class like '%" +
        u_classes +
        "%'"
      this.query(sql)
        .then(result => {
  ...
  static sethealthMod(
    ...
    return new Promise((resolve, reject) => {
      let sql = `insert into health(
          u_id, temperature, hot, gohubei, 
          hubeiren, fever, leaveout, hesuan, 
          mask, masknum, kills,createtime
        ) 
        values (?,?,?,?,?,?,?,?,?,?,?,?)`
  ...
  static gethealthNowDayPageMod(newDate, lastDate, currPage, pageNum) {
    ...
    return new Promise((resolve, reject) => {
      let sql =
        'select * from health where createtime between ? and ? LIMIT  ?,?'
  ...
  static getHealthMonthByidMod(u_id, start, end) {
    return new Promise((resolve, reject) => {
      let sql =
        'select * from health where u_id=? and createtime between ? and ?'
      this.query(sql, this.formatParams(u_id, start, end))
        .then(result => {
          resolve(result)
        })
        .catch(err => {
  ...
  static gethealthNowDayPageTotal(newDate, lastDate) {
    return new Promise((resolve, reject) => {
      let sql =
        'select count(1) as count  from health where createtime between ? and ?'
  ...
  static getHealthNowDayByidMod(u_id, newDate, lastDate) {
    return new Promise((resolve, reject) => {
      let sql =
        'select * from health where (createtime between ? and ?) and u_id = ?'
      this.query(sql, this.formatParams(newDate, lastDate, u_id))
        .then(result => {
          resolve(result)
        })
        .catch(err => {
  ...
  static gethealthNowDayMod(nowDate, lasDate) {
    return new Promise((resolve, reject) => {
      ...
      this.query(sql, this.formatParams(nowDate, lasDate))
        .then(result => {
          resolve(result)
        })
        .catch(err => {
  ...
  static setLeaveMod(
    ...
    return new Promise((resolve, reject) => {
      let sql =
        'insert into `leave` (u_id, classes, reason, leavetype, starttime, endtime,state,createtime) values (?,?,?,?,?,?,0,?) '
      this.query(
        sql,
        this.formatParams(
          u_id,
