...
module.exports = class users_mod extends require('./model') {
  ...
  static LoginUserByid(id, password, type) {
    ...
    return new Promise((resolve, reject) => {
      let sql =
        "select * from user where binary id='" +
        id +
        "' and password='" +
        password +
        "' and type= " +
  ...
  static inXlsxData(inXlsxArr) {
    return new Promise((resolve, reject) => {
      for (let i = 0; i < inXlsxArr.length; i++) {
        let sql =
          'insert into user (id,username,password,head,address,sex,classes,type) values(' +
          inXlsxArr[i].id +
          ", '" +
   ...
   * @param type
   */
  static upUserdataMod(u_id, username, sex, address, type, classes, position) {
    let currTime = tools.getDate19()
    return new Promise((resolve, reject) => {
      let sql = `
        update user set username='${username}', sex='${sex}', address='${address}', type='${type}', position='${position}'
         ${
           classes ? `,classes='${classes}'` : ''
         }, modifytime='${currTime}' where id=${u_id}
        `
      this.query(sql)
        .then(result => {
          resolve('更新成功')
  ...
  static upPwdMod(u_id, oldpassword, newpassword) {
    return new Promise((resolve, reject) => {
      ...
      this.query(sql, this.formatParams(newpassword, oldpassword, u_id))
        .then(result => {
          resolve(result)
        })
        .catch(err => {
  ...
  static upUserHeadMod(head_imgUrl, u_id) {
    return new Promise((resolve, reject) => {
      let sql =
        "update `user` set head= '" + head_imgUrl + "' where id = " + u_id
      this.query(sql)
        .then(result => {
          resolve('上传成功')
        })
